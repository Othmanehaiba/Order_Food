  ...
  function generateSectionDetails(product) {
    return `
      ...
      <div class="md:w-1/2 order-2 md:order-1 flex flex-col">
        ...
        <p class="hidden md:block text-sm text-gray-700 italic mb-4">${product.category}</p>
        <p class="hidden md:block text-sm text-gray-700 mb-6 max-w-md leading-relaxed">${product.description}</p>
        <p class="hidden md:block md:text-center text-gray-700 font-light mb-1">Base price</p>
        <p class="hidden md:block md:text-center text-4xl font-semibold text-yellow-500 mb-8">${product.basePrice}</p>
        <h2 class="hidden md:block text-3xl font-normal mb-6">Customization Options</h2>

        ${product.sizes ? `<div class="hidden md:flex ml-6 mb-6 space-x-6 gap-4">
      ...
      <div class="md:w-1/2 order-1 md:order-2 flex flex-col items-center md:items-start">
        <h1 class="md:hidden text-2xl font-bold mb-2 text-gray-900 text-center">${product.name}</h1>
        <p class="md:hidden text-sm text-gray-700 italic mb-4 text-center">${product.category}</p>
        <img src="${product.images[0]}" alt="${product.name}" class="w-full max-w-sm md:max-w-md h-auto rounded-lg shadow-md mb-4"/>
        <div class="flex space-x-2 md:space-x-3 justify-center md:justify-start w-full overflow-hidden mb-4 md:mb-6">
          <img src="../assets/french-fries.jpg" alt="Ingredient 1" class="h-[90px] md:h-[120px] basis-[22%] md:basis-[20%] rounded-lg object-cover shadow"/>
          <img src="../assets/french-fries.jpg" alt="Ingredient 2" class="h-[90px] md:h-[120px] basis-[22%] md:basis-[20%] rounded-lg object-cover shadow"/>
          <img src="../assets/french-fries.jpg" alt="Ingredient 3" class="h-[90px] md:h-[120px] basis-[22%] md:basis-[20%] rounded-lg object-cover shadow"/>
        </div>
        <p class="md:hidden text-sm text-gray-700 mb-6 text-center max-w-md leading-relaxed">${product.description}</p>
        <p class="md:hidden text-xl text-gray-700 font-light mb-1 text-center">Base price</p>
        <p class="md:hidden text-4xl font-semibold text-yellow-500 mb-6 text-center">${product.basePrice}</p>
        ${product.sizes ? `<div class="flex md:hidden ml-6 mb-6 space-x-6 gap-4">
...
if(priceEl) priceEl.textContent = `${product.basePrice}`;
  ...
  const updatePrice = (quantity) => {
    const total = ((product.basePrice + selectedSizePrice) * quantity).toFixed(2);
  ...
  qtyBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      let currentVal = parseInt(quantityDesktop?.value || quantityMobile?.value || "1") || 1;
